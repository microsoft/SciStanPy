...
Compehensive documentation for the SciStanPy API can be found :doc:`here <docs/api/index>`. For starting quickly, however, the most important modules and objects are :doc:`model <docs/api/model/model>`, :doc:`parameters <docs/api/model/components/parameters>`, and :doc:`operations <docs/api/operations>`. Less frequently used, but also important, is the :doc:`constants <docs/api/model/components/constants>` module. Follow the links for detailed API references. In short, however, the :py:class:`~scistanpy.model.model.Model` object forms the backbone of all SciStanPy models, distributions defined in :doc:`parameters <docs/api/model/components/parameters>` define those models' variables, :doc:`operations <docs/api/operations>` define transformations of those variables, and :doc:`constants <docs/api/model/components/constants>` provide fixed values used throughout the model.
...
Additional information on the dashboard can be found :doc:`here <docs/api/plotting/prior_predictive>`.
...
Additional methods exposed by the :py:class:`~scistanpy.model.results.mle.MLEInferenceRes` can be found in its :doc:`associated documentation <docs/api/model/results/mle>`. Note that the MLEInferenceRes class also exposes the `inference_obj` instance variable, which is an `arviz.InferenceData <https://python.arviz.org/en/latest/api/generated/arviz.InferenceData.html>_` object containing the samples drawn from the model. This can be used to interface directly with the `ArviZ ecosystem <https://python.arviz.org/en/latest/index.html>_` for analysis of Bayesian models.
...
Additional examples demonstrating usage of the above features can be found in the :doc:`docs/examples/index` section of the documentation.
